...
try:
    ...
    if all_results:  # 只要有无抑制结果就执行
        ...
        if inhibition_types:
            ...
            if key in all_results and key != "no_inhibition":  # 确保不是无抑制
                t_hour, L, Gal, rates = all_results[key]
        ...
        ax2.annotate(annotation_text_no_inh,
                     ...
                     xytext=(L_no_inh[max_rate_idx_no_inh] + 0.05 * L0, max_rate_no_inh * 0.8),  # 向下调整
        ...
        ax2.set_xlim([0, L0])
        ax2.set_ylim([0, y_max])   # 使用动态设置的y_max
